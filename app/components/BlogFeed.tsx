          ...
          {posts.map(p => (
            <Link key={p.id} href={`/blog/${p.slug}`} className="bg-[#F2EDE4] group flex flex-col">
              <div className="aspect-[4/3] overflow-hidden bg-black/5">
                {p.cover_url ? (
                  <img src={cdnUrl(p.cover_url, "w_800,c_limit,q_auto,f_auto")} alt={p.title} loading="lazy" decoding="async" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                ) : (
